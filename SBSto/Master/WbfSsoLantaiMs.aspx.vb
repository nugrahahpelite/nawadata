...
Public Class WbfSsoLantaiMs
    ...
    Private Sub psClearMessage()
        LblMsgLantaiDescr.Visible = False
    ...
    Private Sub psDisplayData()
        ...
        vnQuery = "Select * From " & fbuGetDBMaster() & "Sys_Lantai_MA Where OID=" & TxtOID.Text
        ...
        Else
            ...
            TxtLantaiDescr.Text = fbuValStr(vnDtb.Rows(0).Item("LantaiDescription"))
    ...
    Private Sub psEnableInput(vriBo As Boolean)
        TxtLantaiDescr.ReadOnly = Not vriBo
        LblMsgErrorNE.Visible = False
    End Sub
    ...
    Protected Sub BtnSimpan_Click(sender As Object, e As EventArgs) Handles BtnSimpan.Click
        ...
        If Len(Trim(TxtLantaiDescr.Text)) = 0 Then
            LblMsgLantaiDescr.Text = "Isi Deskripsi Lantai"
            LblMsgLantaiDescr.Visible = True
            vnSave = False
        End If
        Try
            Dim vnSQLConn As New SqlConnection
            ...
            Dim vnLantaiDescr As String = fbuFormatString(Trim(TxtLantaiDescr.Text))
            If HdfActionStatus.Value = cbuActionNew Then
                vnQuery = "Select count(*) from " & vnDBMaster & "Sys_Lantai_MA Where LantaiDescription='" & vnLantaiDescr & "'"
                If fbuGetDataNumSQL(vnQuery, vnSQLConn) > 0 Then
                    LblMsgLantaiDescr.Text = "Deskripsi Lantai " & Trim(TxtLantaiDescr.Text) & " Sudah digunakan" & vbCrLf & "Silakan Cek Daftar Deskripsi Lantai"
                    LblMsgLantaiDescr.Visible = True
                    vnSave = False
                End If
                If Not vnSave Then
                ...
                vnQuery = "Insert into " & vnDBMaster & "Sys_Lantai_MA("
                vnQuery += vbCrLf & "LantaiDescription,"
                ...
                vnQuery = "Select OID From " & vnDBMaster & "Sys_Lantai_MA Where LantaiDescription='" & vnLantaiDescr & "'"
                vnOID = fbuGetDataNumSQLTrans(vnQuery, vnSQLConn, vnSQLTrans)

                TxtOID.Text = vnOID
            Else
                vnQuery = "Select count(*) from " & vnDBMaster & "Sys_Lantai_MA Where LantaiDescription='" & vnLantaiDescr & "' and OID<>" & TxtOID.Text
                If fbuGetDataNumSQL(vnQuery, vnSQLConn) > 0 Then
                    LblMsgLantaiDescr.Text = "Deskripsi Lantai " & Trim(TxtLantaiDescr.Text) & " Sudah digunakan" & vbCrLf & "Silakan Cek Daftar Deskripsi Lantai"
                    LblMsgLantaiDescr.Visible = True
                    vnSave = False
                ...
                vnQuery = "Update " & vnDBMaster & "Sys_Lantai_MA set "
                vnQuery += vbCrLf & "LantaiDescription='" & vnLantaiDescr & "',"
    ...
    Protected Sub GrvList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GrvList.SelectedIndexChanged
        TxtOID.Text = GrvList.SelectedRow.Cells(1).Text
        psDisplayData()
        psEnableInput(False)
        psEnableSave(False)
        HdfActionStatus.Value = cbuActionNorm
    ...
    Private Sub psFillGrvList()
        ...
        Dim vnDtb As New DataTable
        Dim vnQuery As String
        vnQuery = "Select PM.LantaiDescription,"
        ...
        vnQuery += vbCrLf & " From " & vnDBMaster & "Sys_Lantai_MA PM"
        ...
        vnQuery += vbCrLf & "      Where PM.LantaiDescription like '%" & vnCriteria & "%'"
        vnQuery += vbCrLf & " Order by PM.LantaiDescription"
        pbuFillDtbSQL(vnDtb, vnQuery, vnSQLConn)
