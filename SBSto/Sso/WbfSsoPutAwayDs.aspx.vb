...
Public Class WbfSsoPutAwayDs
    Inherits System.Web.UI.Page

    Const csModuleName = "WbfSsoPutAwayDs"
    Const csTNoPrefix = "PW"
    ...
    Enum ensColData1
        ...
        vDelItem1 = 5
        vPWScan1Deleted = 6
        vPWScan1DeletedUser = 7
        PWScan1DeletedNote = 8
        vPWScan1DeletedTime = 9
    End Enum
    ...
    Enum ensColData2
        ...
        vDelItem2 = 6
        vPWScan2Deleted = 7
        vPWScan2DeletedUser = 8
        PWScan2DeletedNote = 9
        vPWScan2DeletedTime = 10
    End Enum
    ...
    Private Sub psFillGrvList()
        ...
        If ChkSt_Baru.Checked = True Then
            vnCrStatus += enuTCPWAY.Baru & ","
        End If
        If ChkSt_OnPutaway.Checked = True Then
            vnCrStatus += enuTCPWAY.On_Putaway & ","
        End If
        If ChkSt_PutawayDone.Checked = True Then
            vnCrStatus += enuTCPWAY.Putaway_Done & ","
        End If
        If ChkSt_Cancelled.Checked = True Then
            vnCrStatus += enuTCPWAY.Cancelled & ","
        End If
        If vnCrStatus <> "" Then
        ...
        vnQuery = "Select pwh.OID,pwh.PWCompanyCode,pwh.PWNo,rcv.RcvPONo,convert(varchar(11),pwh.PWDate,106)vPWDate,"
        vnQuery += vbCrLf & "       str.WarehouseName,pwh.PWCancelNote,stn.TransStatusDescr,"
        ...
        Else
            vnQuery += vbCrLf & "     inner join Sys_SsoUserCompany_MA uc with(nolock) on uc.CompanyCode=pwh.PWCompanyCode and uc.UserOID=" & Session("UserOID")
        End If
        ...
        If Trim(TxtListNo.Text) <> "" Then
            vnQuery += vbCrLf & " and pwh.PWNo like '%" & Trim(TxtListNo.Text) & "%'"
        End If
        ...
        vnQuery += vbCrLf & "     Order by pwh.PWNo"
    ...
    Private Sub psDisplayData(vriSQLConn As SqlConnection)
        ...
        vnQuery = "Select pwh.OID,pwh.PWNo,pwh.PWCompanyCode,pwh.WarehouseOID,pwh.StorageOID,convert(varchar(11),pwh.PWDate,106)vPWDate,"
        vnQuery += vbCrLf & "      whs.WarehouseName,pwh.PWDoneNote,pwh.RcvPOHOID,rch.RcvPONo,pwh.TransStatus,ST.TransStatusDescr"
        vnQuery += vbCrLf & " From Sys_SsoPWHeader_TR pwh"
        ...
        If vnDtb.Rows.Count = 0 Then
            psClearData()
        Else
            TxtTransDate.Text = vnDtb.Rows(0).Item("vPWDate")
            TxtTransNo.Text = vnDtb.Rows(0).Item("PWNo")
            TxtTransWhsName.Text = vnDtb.Rows(0).Item("WarehouseName")
            TxtTransRefNo.Text = vnDtb.Rows(0).Item("RcvPONo")

            TxtCompany.Text = vnDtb.Rows(0).Item("PWCompanyCode")
    ...
    Private Sub psFillGrvSumm(vriEmpty As Byte, vriPWHOID As String, vriSQLConn As SqlClient.SqlConnection)
        ...
        If vriEmpty = 1 Then
            vnQuery = "Select ''BRGCODE,''BRGNAME,0 vSumPWScan1Qty,0 vSumPWScan2Qty Where 1=2"
        Else
            vnQuery = "Select mb.BRGCODE,mb.BRGNAME,pws1.vSumPWScan1Qty,pws2.vSumPWScan2Qty"
            vnQuery += vbCrLf & "From fnTbl_SsoPWHeaderScan1(" & vriPWHOID & ",0) pws1"
            vnQuery += vbCrLf & "     inner join " & fbuGetDBMaster() & "Sys_MstBarang_MA mb with(nolock) on mb.BRGCODE=pws1.BRGCODE and mb.CompanyCode=pws1.PWCompanyCode"
            vnQuery += vbCrLf & "     left outer join fnTbl_SsoPWHeaderScan2(" & vriPWHOID & ",0) pws2 on pws2.PWHOID=pws1.PWHOID and pws2.BRGCODE=pws1.BRGCODE"

            vnQuery += vbCrLf & " Where (mb.BRGCODE like '%" & vnCriteria & "%' or mb.BRGNAME like '%" & vnCriteria & "%')"
            vnQuery += vbCrLf & "order by case when isnull(vSumPWScan2Qty,0)=0 then 5 else 1 end,mb.BRGCODE"
        End If
    ...
    Private Sub psFillGrvData1(vriEmpty As Byte, vriBrgCode As String, vriSQLConn As SqlClient.SqlConnection)
        ...
        If vriEmpty = 1 Then
            vnQuery = "Select 0 OID,0 PWScan1Qty,"
            vnQuery += vbCrLf & "       ''PWScan1Note,"
            vnQuery += vbCrLf & "       ''vPWScan1User,"
            vnQuery += vbCrLf & "	    ''vPWScan1Time,"
            vnQuery += vbCrLf & "	    ''vPWScan1Deleted,''vPwScan1DeletedUser,''PWScan1DeletedNote,"
            vnQuery += vbCrLf & "	    ''vPWScan1DeletedTime,"
            vnQuery += vbCrLf & "	    ''vDelItem1 Where 1=2"
        Else
            vnQuery = "Select sc.OID,sc.PWScan1Qty,"
            vnQuery += vbCrLf & "       sc.PWScan1Note,"
            vnQuery += vbCrLf & "       mu.UserName vPWScan1User,"
            vnQuery += vbCrLf & "	    convert(varchar(11),sc.PWScan1Datetime,106) + ' ' + convert(varchar(5),sc.PWScan1Datetime,108)vPWScan1Time,"
            vnQuery += vbCrLf & "	    case when abs(PWScan1Deleted)=1 then 'Y' else 'N' end vPWScan1Deleted,"
            vnQuery += vbCrLf & "       md.UserName vPWScan1DeletedUser,"
            vnQuery += vbCrLf & "	    PWScan1DeletedNote,"
            vnQuery += vbCrLf & "	    convert(varchar(11),sc.PWScan1DeletedDatetime,106) + ' ' + convert(varchar(5),sc.PWScan1DeletedDatetime,108)vPWScan1DeletedTime,"
            vnQuery += vbCrLf & "	    ''vDelItem1"
            vnQuery += vbCrLf & "  From Sys_SsoPWScan1_TR sc with(nolock)"
            vnQuery += vbCrLf & "	    inner join Sys_SsoUser_MA mu with(nolock) on mu.OID=sc.PWScan1UserOID"
            vnQuery += vbCrLf & "	    left outer join Sys_SsoUser_MA md with(nolock) on md.OID=sc.PwScan1DeletedUserOID"
            vnQuery += vbCrLf & " Where sc.PWHOID=" & TxtTransID.Text & " and sc.BrgCode='" & fbuFormatString(vriBrgCode) & "'"
            vnQuery += vbCrLf & "       and sc.PWScan1Note like '%" & vnCriteria & "%'"

            If Not (ChkSt_DelNo.Checked = True And ChkSt_DelYes.Checked = True) Then
                If ChkSt_DelNo.Checked = True Then
                    vnQuery += vbCrLf & "       and abs(PWScan1Deleted)=0"
                Else
                    vnQuery += vbCrLf & "       and abs(PWScan1Deleted)=1"
                End If
            End If
        ...
        If ChkSt_DelYes.Checked = True Then
            ...
            For vn = 0 To GrvData1.Rows.Count - 1
                If GrvData1.Rows(vn).Cells(ensColData1.vPWScan1Deleted).Text = "Y" Then
    ...
    Private Sub psFillGrvData2(vriEmpty As Byte, vriStorageOID As String, vriBrgCode As String, vriSQLConn As SqlClient.SqlConnection)
        ...
        If vriEmpty = 1 Then
            vnQuery = "Select 0 OID,''vStorageInfoHtml,0 PWScan2Qty,"
            vnQuery += vbCrLf & "       ''PWScan2Note,"
            vnQuery += vbCrLf & "       ''vPWScan2User,"
            vnQuery += vbCrLf & "	    ''vPWScan2Time,"
            vnQuery += vbCrLf & "	    ''vPWScan2Deleted,''vPwScan2DeletedUser,''PWScan2DeletedNote,"
            vnQuery += vbCrLf & "	    ''vPWScan2DeletedTime,"
            vnQuery += vbCrLf & "	    ''vDelItem2 Where 1=2"
        Else
            vnQuery = "Select sc.OID,st.vStorageInfoHtml,sc.PWScan2Qty,"
            vnQuery += vbCrLf & "       sc.PWScan2Note,"
            vnQuery += vbCrLf & "       mu.UserName vPWScan2User,"
            vnQuery += vbCrLf & "	    convert(varchar(11),sc.PWScan2Datetime,106) + ' ' + convert(varchar(5),sc.PWScan2Datetime,108)vPWScan2Time,"
            vnQuery += vbCrLf & "	    case when abs(PWScan2Deleted)=1 then 'Y' else 'N' end vPWScan2Deleted,"
            vnQuery += vbCrLf & "       md.UserName vPWScan2DeletedUser,"
            vnQuery += vbCrLf & "	    PWScan2DeletedNote,"
            vnQuery += vbCrLf & "	    convert(varchar(11),sc.PWScan2DeletedDatetime,106) + ' ' + convert(varchar(5),sc.PWScan2DeletedDatetime,108)vPWScan2DeletedTime,"
            vnQuery += vbCrLf & "	    ''vDelItem2"
            vnQuery += vbCrLf & "  From Sys_SsoPWScan2_TR sc with(nolock)"
            vnQuery += vbCrLf & "	    inner join " & fbuGetDBMaster() & "fnTbl_SsoStorageData('') st on st.vStorageOID=sc.StorageOID"
            vnQuery += vbCrLf & "	    inner join Sys_SsoUser_MA mu with(nolock) on mu.OID=sc.PWScan2UserOID"
            vnQuery += vbCrLf & "	    left outer join Sys_SsoUser_MA md with(nolock) on md.OID=sc.PwScan2DeletedUserOID"
            vnQuery += vbCrLf & " Where sc.PWHOID=" & TxtTransID.Text & " and sc.BrgCode='" & fbuFormatString(vriBrgCode) & "'"
            ...
            vnQuery += vbCrLf & "       and sc.PWScan2Note like '%" & vnCriteria & "%'"

            If Not (ChkSt_DelNo.Checked = True And ChkSt_DelYes.Checked = True) Then
                If ChkSt_DelNo.Checked = True Then
                    vnQuery += vbCrLf & "       and abs(PWScan2Deleted)=0"
                Else
                    vnQuery += vbCrLf & "       and abs(PWScan2Deleted)=1"
                End If
            End If
            vnQuery += vbCrLf & " Order by sc.OID"
        End If
        ...
        If ChkSt_DelYes.Checked = True Then
            ...
            For vn = 0 To GrvData2.Rows.Count - 1
                If GrvData2.Rows(vn).Cells(ensColData2.vPWScan2Deleted).Text = "Y" Then
    ...
    Protected Sub BtnCancelPW_Click(sender As Object, e As EventArgs) Handles BtnCancelPW.Click
    ...
    Protected Sub BtnConfirmYes_Click(sender As Object, e As EventArgs) Handles BtnConfirmYes.Click
        If HdfProcess.Value = "CancelPW" Then
    ...
    Private Sub psButtonStatus()
        ...
        Else
            BtnCancelPW.Enabled = (HdfTransStatus.Value = enuTCPWAY.Baru Or HdfTransStatus.Value = enuTCPWAY.On_Putaway)
            psButtonVisible()
        End If
    End Sub
    Private Sub psButtonVisible()
        BtnCancelPW.Visible = BtnCancelPW.Enabled
        BtnList.Visible = BtnList.Enabled
    End Sub

    Private Sub psCancelPW()
        ...
        Try
            Dim vnPWHOID As String = TxtTransID.Text
            Dim vnTransStatus As Integer
            Dim vnCount1 As Integer
            Dim vnQuery As String
            vnQuery = "Select TransStatus From Sys_SsoPWHeader_TR with(nolock) Where OID=" & vnPWHOID
            ...
            If vnTransStatus = enuTCPWAY.Cancelled Or vnTransStatus > enuTCPWAY.On_Putaway Then
            ...
            vnQuery = "Select count(1) FROM Sys_SsoPWScan1_TR Where PWHOID=" & vnPWHOID & " and abs(PWScan1Deleted)=0"
            vsTextStream.WriteLine("")
            vsTextStream.WriteLine("0.1")
            vsTextStream.WriteLine("vnQuery")
            vsTextStream.WriteLine(vnQuery)
            ...
            vnQuery = "Update Sys_SsoPWHeader_TR set TransStatus=" & enuTCPWAY.Cancelled & ",PWCancelNote='" & fbuFormatString(Trim(TxtConfirmNote.Text)) & "',"
            vnQuery += vbCrLf & "CancelledUserOID=" & Session("UserOID") & ",CancelledDatetime=getdate() Where OID=" & vnPWHOID
            vsTextStream.WriteLine("")
            ...
            vsTextStream.WriteLine("pbuInsertStatusPW...Start")
            pbuInsertStatusPW(vnPWHOID, enuTCPWAY.Cancelled, Session("UserOID"), vnSQLConn, vnSQLTrans)
            vsTextStream.WriteLine("pbuInsertStatusPW...End")
