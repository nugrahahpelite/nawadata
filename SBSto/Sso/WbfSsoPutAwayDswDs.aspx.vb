...
Public Class WbfSsoPutAwayDswDs
    Inherits System.Web.UI.Page


    Const csModuleName = "WbfSsoPutAwayDswDs"
    ...
    Private Sub psFillGrvList()
        ...
        If ChkSt_OnDispatch.Checked = False And ChkSt_DispatchDone.Checked = False Then
        ...
        vnQuery += vbCrLf & "Order by pwh.DSRDate DESC"
    ...
    Protected Sub BtnDataClose_Click(sender As Object, e As EventArgs) Handles BtnDataClose.Click
        psShowData(False)
    End Sub
    ...
    Private Sub psShowData(vriBo As Boolean)
        If vriBo Then
    ...
    Private Sub psFillGrvLsPick(vriEmpty As Byte, vriSQLConn As SqlConnection)
        ...
        If vriEmpty = 1 Then
            vnQuery = "Select 0 OID,''PCKNo,''vPCKDate,''PCLRefHNo,''SchDTypeName,''vIsQtyConfirm,''TransStatusDescr,''PCLNo,''vDelItem Where 1=2"
        Else
            vnQuery = "Select pwh.OID vPCKHOID,pwh.PCKNo,convert(varchar(11),pwh.PCKDate,106)vPCKDate,pch.PCLRefHNo,msc.SchDTypeName,"
            ...
            vnQuery += vbCrLf & "       pch.PCLNo,''vDelItem"
            ...
            vnQuery += vbCrLf & " Order by pch.PCLRefHNo"
        End If
