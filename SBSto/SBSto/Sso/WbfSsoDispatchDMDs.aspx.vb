...
Public Class WbfSsoDispatchDMDs
    ...
    Private Sub psDefaultDisplay()
        ...
        DivSKK.Style(HtmlTextWriterStyle.Visibility) = "hidden"
        PanSKK.Style(HtmlTextWriterStyle.Position) = "absolute"
    ...
    Private Sub psFillGrvList()
        ...
        vnQuery = "Select pwh.OID,pwh.DSPCompanyCode,pwh.DSPNo,sch.DcmSchNo,convert(varchar(11),pwh.DSPDate,106)vDSPDate,"
        ...
        vnQuery += vbCrLf & "       left outer join " & vnDBDcm & "Sys_DcmScheduleHeader_TR sch with(nolock) on sch.OID=pwh.DcmSchHOID"
        ...
        If Trim(TxtListNo.Text) <> "" Then
            vnQuery += vbCrLf & " and (pwh.DSPNo like '%" & Trim(TxtListNo.Text) & "%' or sch.DcmSchNo like '%" & Trim(TxtListNo.Text) & "%')"
        End If
    ...
    Private Sub psDisplayData(vriSQLConn As SqlConnection)
        ...
        vnQuery = "Select pwh.*,convert(varchar(11),pwh.DSPDate,106)vDSPDate,sch.DcmSchNo,"
        ...
        Else
            ...
            TxtSKKOID.Text = fbuValStr(vnDtb.Rows(0).Item("DcmSchHOID"))
            TxtSKKNo.Text = fbuValStr(vnDtb.Rows(0).Item("DcmSchNo"))
    ...
    Private Sub psSaveEdit()
        ...
        Try
            ...
            Dim vnQuery As String
            Dim vnDcmSchHOID As String = HdfDcmSchHOID.Value
            Dim vnDriverOID As String = HdfDcmSchDriverOID.Value
            Dim vnVehicleOID As String = HdfDcmVehicleOID.Value

            vnSQLTrans = vnSQLConn.BeginTransaction()
            vnBeginTrans = True

            vnQuery = "Update Sys_SsoDSPHeader_TR set"
            vnQuery += vbCrLf & "DcmSchHOID=" & vnDcmSchHOID & ","
    ...
    Protected Sub BtnSKK_Click(sender As Object, e As EventArgs) Handles BtnSKK.Click
        psFillGrvSKK()
        psShowSKK(True)
    End Sub

    Private Sub psShowSKK(vriBo As Boolean)
        If vriBo Then
            PanSKK.Style(HtmlTextWriterStyle.Top) = "85px"
            PanSKK.Style(HtmlTextWriterStyle.Left) = "15px"
            PanSKK.Style(HtmlTextWriterStyle.Width) = "850px"
            PanSKK.Style(HtmlTextWriterStyle.Height) = "850px"

            DivSKK.Style(HtmlTextWriterStyle.Visibility) = "visible"
        Else
            DivSKK.Style(HtmlTextWriterStyle.Visibility) = "hidden"
        End If
    End Sub
    Private Sub psFillGrvSKK()
        Dim vnSQLConn As New SqlConnection
        If Not fbuConnectSQL(vnSQLConn) Then
            LblSKKError.Text = pbMsgError
            LblSKKError.Visible = True
            pbMsgError = ""
        ...
        Dim vnQuery As String
        Dim vnDtb As New DataTable
        Dim vnCrList As String = fbuFormatString(Trim(TxtSKKFind.Text))

        vnQuery = "	Select DISTINCT dsh.OID,whs.warehouseName, dsh.DcmSchNo, dsh.DcmschDate , dsh.DcmschLeaveDate,"
        vnQuery += vbCrLf & "	  md.DcmDriverName, dsh.DcmSchDriverOID, dsh.VehicleOID,"
        vnQuery += vbCrLf & "	  MV.VehicleNo, mvt.VehicleTyeDescr,dsh.SchHTypeOID"
        vnQuery += vbCrLf & "From " & vnDBDcm & "Sys_DcmScheduleHeader_TR dsh with(nolock)	"
        vnQuery += vbCrLf & "	  inner join " & vnDBMaster & "Sys_Warehouse_MA whs with(nolock) on whs.WarehouseCode = dsH.LocationCode"

        vnQuery += vbCrLf & "	  left outer join " & vnDBDcm & "Sys_DcmDriver_MA md on md.OID = dsh.DcmSchDriverOID"
        vnQuery += vbCrLf & "	  left outer join " & vnDBDcm & "Sys_DcmVehicle_MA mv on mv.OID = dsh.VehicleOID"
        vnQuery += vbCrLf & "	  left outer join " & vnDBDcm & "Sys_DcmVehicleType_MA mvt on mv.VehicleTypeOID = mvt.OID"
        vnQuery += vbCrLf & "     inner join " & vnDBDcm & "Sys_DcmUserWarehouse_MA usw ON  whs.OID = usw.WarehouseOID"
        vnQuery += vbCrLf & "Where dsh.TransStatus=0"
        If vnCrList = "" Then
            vnQuery += vbCrLf & "           and dsh.DcmSchNo like '%" & vnCrList & "%'"
        End If

        vnQuery += vbCrLf & "     Order by dsh.DcmschDate desc"
        pbuFillDtbSQL(vnDtb, vnQuery, vnSQLConn)
        GrvSKK.DataSource = vnDtb
        GrvSKK.DataBind()
    ...
    Private Sub GrvSKK_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GrvSKK.RowCommand
        If e.CommandName = "DcmSchNo" Then
            Dim vnIdx As Integer = Convert.ToInt32(e.CommandArgument)
            If vnIdx >= GrvSKK.Rows.Count Then Exit Sub

            Dim vnGRow As GridViewRow = GrvSKK.Rows(vnIdx)
            If vnIdx >= GrvSKK.Rows.Count Then Exit Sub

            HdfDcmSchHOID.Value = vnGRow.Cells(ensColSKK.OID).Text
            HdfDcmSchDriverOID.Value = vnGRow.Cells(ensColSKK.DcmSchDriverOID).Text
            HdfDcmVehicleOID.Value = vnGRow.Cells(ensColSKK.VehicleOID).Text

            TxtSKKOID.Text = vnGRow.Cells(ensColSKK.OID).Text
            TxtSKKNo.Text = vnGRow.Cells(ensColSKK.DcmSchNo).Text
            TxtTransDriver.Text = vnGRow.Cells(ensColSKK.DcmDriverName).Text
            TxtTransVehicle.Text = vnGRow.Cells(ensColSKK.VehicleNo).Text

            psShowSKK(False)
        End If
    End Sub

    Protected Sub BtnSKKClose_Click(sender As Object, e As EventArgs) Handles BtnSKKClose.Click
        psShowSKK(False)
    End Sub
End Class